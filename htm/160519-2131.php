...
//$url=$input_a;
if(preg_match('/http/', $url)){//檢查url格式
	//
}else{
	if(preg_match('/http/', $input_a)){//檢查url格式
		$url=$input_a;
	}
}


//$htmlbody='url='.$url.'';phppoi();exit;
//echo $url;exit;
//載入外部檔案
...
if($kdao_only){//只使用於綜合網址
	...
	if(!$cc){
		...
		$url_p1=parse_url($url);
		$FFF_ary = explode("=", $url_p1['query']);
		$FFF='http://archive.komica.org/00/index.php?res='.$FFF_ary[1];
		
		$htmlbody='[x]blockquote'.'=>'."<a href='$phpself?$FFF'>archive</a>";
		//print_r($FFF_ary[1],true);
		phppoi();exit;
	}
	...
	foreach($chat_array as $k => $v){//迴圈
		...
		$htmlbody.= '<span class="num">#'.$cc.'</span>'." ";
		//名稱
		...
		if( $v['image'] ){//回應中有圖 // 網址字串
			...
			if(1==1){
				...
				if($cc%2){
					$tmp=$tmp_w;
					$tmp_a="https://demo.cloudimg.io/cdn/n/n/".$tmp;
				}else{
				...
				if( preg_match('/\.webm$/',$v['image'])){
					...
					$htmlbody.= '<img class="zoom" src="'.$tmp_w2.'"/>';
					$htmlbody.= '<video controls preload="metadata" src="'.$tmp_w2.'">[video]</video>';
					$htmlbody.='<br/>'.$tmp_w2;
				}else{
					//$tmp="http://demo.cloudimg.io/cdn/n/n/".$tmp0;
			...
			if(1==0){
				$tmp0="http://web.archive.org/web/2016/".$v['image'];
				if( preg_match('/\.webm$/',$v['image']) && 0){
					$tmp="".$tmp0;
					$htmlbody.= '[<span class="image"><video controls >您<source src="'.$tmp.'" type="video/webm"></video></span>]';
				}else{
					$tmp="http://demo.cloudimg.io/cdn/n/n/".$tmp0;
					$htmlbody.= '[<span class="image"><img class="zoom" src="'.$tmp.'"/></span>]';
				}
				//$tmp="".$tmp0;
				$htmlbody.= '[<span class="image2"><a href="'.$tmp.'"/>備份?</a></span>]';
				if( preg_match('/\.webm$/',$v['image']) ){$htmlbody.='<b>webm內容</b>';}
			}
			if(1==0){
				$FFF_arr=explode("//",$v['image']);
				$FFF=$cc2%3;
				$tmp='https://i'.$FFF.'.wp.com/'.$FFF_arr[1];
				
				$tmp="http://demo.cloudimg.io/cdn/n/n/".$tmp;
				$htmlbody.= '[<span class="image"><img class="zoom" src="'.$tmp.'"/></span>]';
			}
			if(1==0){//wp用
				$tmp0="http://web.archive.org/web/2016/".$v['image'];
				...
				if( preg_match('/\.webm$/',$v['image']) ){
					$tmp="".$tmp0;
					$htmlbody.= '[<span class="image"><video controls autoplay	>您<source src="'.$tmp.'" type="video/webm"></video></span>]';
				}else{
					$tmp="".$tmp0;
					$htmlbody.= '[<span class="image"><img class="zoom" src="'.$tmp.'"/></span>]';
				}
				$tmp="https://crossorigin.me/".$tmp0;
				$htmlbody.= '[<span class="image2"><a href="'.$tmp0.'"/>備份?</a></span>]';
				//$htmlbody.= 'wp-ah';
				if( preg_match('/\.webm$/',$v['image']) ){$htmlbody.='<b>webm內容</b>';}
			}
		...
		if($cc==1){
			//$htmlbody.= 'wp+ah'."\n";
			$htmlbody.= '<hr class="trumbowyg-more">'."\n";
			$htmlbody.= '<!--entrybreak-->'."\n";
			$htmlbody.= '<!--more-->'."\n";
...
*/
	...
	//if(is_file($tmp)){unlink($tmp);if(is_file($tmp)){die("刪除檔案失敗");}}//舊檔案有存在就移除
...
function phppoi(){
	...
	$output.='<a href="'.$output_path.'?'.$time.'">'.$output_path.'</a>'."\n";
